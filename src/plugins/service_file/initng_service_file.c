...
a_state_h PARSING =
	{ "PARSING", "This is service is parsing by service_file.", IS_NEW,
	NULL,
	NULL, NULL
};
a_state_h PARSING_FOR_START =
	{ "PARSING_FOR_START", "This is service is parsing by service_file.",
	IS_NEW, NULL, NULL, NULL
};
a_state_h NOT_RUNNING = { "NOT_RUNNING",
	"When a service is parsed done, its marked DOWN ready for starting.",
	IS_DOWN, NULL, NULL,
	NULL
};
a_state_h REDY_FOR_START = { "REDY_TO_START",
	"When a service is parsed done, its marked DOWN ready for starting.",
	IS_DOWN, NULL, NULL,
	NULL
};
a_state_h PARSE_FAIL =
	{ "PARSE_FAIL", "This parse process failed.", IS_FAILED, NULL, NULL,
	NULL
};
...
stype_h unset =
	{ "unset", "Service type is not set yet, are still parsing.", FALSE,
	NULL,
	NULL, NULL
};
			    ...
			    const char *value)
{
	...
	case VARIABLE_SET:
		D_("set type\n");
		set_var(type, varname ? initng_toolbox_strdup(varname) : NULL,
			active);
		break;
	case INT:
	case VARIABLE_INT:
			    ...
			    const char *vartype, const char *varname)
{
	...
	case VARIABLE_STRINGS:
		/* todo, will only return one */
		{
			...
			while ((tmp =
				get_next_string_var(type, varname, active,
						    &itt))) {
				if (!rep->message[0]) {
...
static int parse_new_service_file(s_event * event, char *file)
{
	...
	if (initng_fork(new_active, process) == 0) {
		...
		/* SERVICE=getty/tty1 */
		new_env[0] =
			initng_toolbox_calloc(strlen(name) + 20, sizeof(char));
