...
#define is_var(type, vn, point) initng_data_is_var(type, vn, &(point)->data)
...
#define set_var(type, vn, point) initng_data_set_var(type, vn, &(point)->data)
#define set(type, point) set_var(type, NULL, point)


/*
 * initng_data_get_int_var()
 * Gets an integear, with matching type, or 0 if not found.
 */
int initng_data_get_int_var(s_entry * type, const char *vn, data_head * d);

#define get_int_var(type, vn, point) initng_data_get_int_var(type, vn, &(point)->data)
#define get_int(type, point) get_int_var(type, NULL, point)

/*
 * initng_data_set_int_var()
 * Sets an int value, on type.
 */
void initng_data_set_int_var(s_entry * type, char *vn, data_head * d, int value);

#define set_int_var(type, vn, point, value) initng_data_set_int_var(type, vn, &(point)->data, value)
#define set_int(type, point, value) set_int_var(type, NULL, point, value)

/*
 * initng_data_get_string_var()
 * Returns a string, if found.
 */
const char *initng_data_get_string_var(s_entry * type, const char *vn, data_head * d);

#define get_string_var(type, vn, point) initng_data_get_string_var(type, vn, &(point)->data)
#define get_string(type, point) get_string_var(type, NULL, point)

/*
 * initng_data_get_next_string()
...
#define get_next_string_var(type, vn, point, cur) initng_data_get_next_string_var(type, vn, &(point)->data, cur)
#define get_next_string(type, point, cur) get_next_string_var(type, NULL, point, cur)

/*
 * initng_data_set_string()
 * Sets a string.
 */
void initng_data_set_string_var(s_entry * type, char *vn, data_head * d, char *string);

#define set_string_var(type, vn, point, string) initng_data_set_string_var(type, vn, &(point)->data, string)
 ...
 */
void initng_data_set_another_string_var(s_entry * type, char *vn, data_head * d,
							  char *string);
#define set_another_string_var(type, vn, point, string) initng_data_set_another_string_var(type, vn, &(point)->data, string)
 ...
 */
int initng_data_count_type(s_entry * type, data_head * data);

#define initng_data_count_all(point) initng_data_count_type(NULL, point)
#define count_type(type, point) initng_data_count_type(type, &(point)->data)
#define count_all(point) initng_data_count_all(&(point)->data)
...
#define remove_all(point) initng_data_remove_all(&(point)->data)
...
#define copy_all(from, to) initng_data_copy_all(&(from)->data, &(to)->data)


#endif /* INITNG_STRUCT_DATA_H */
