...
static int load_service(const char *name, int level)
{
    ...
    {
        SPACE;
        printf("Probeparsing: %s ...", name);
    }

    service = initng_plugin_create_new_active(name);
    ...
    // If not found, create an fake one in its name.
    if (!service)
    {
        /* unload all modules
        ...
        service = initng_active_db_new(name);
        if (service)
        {
	    // Set type
	    service->type=initng_service_type_get_by_name("service");
	    
	    // Register this one in the database
            initng_active_db_register(service);
    ...
    if (print_each == 1)
    {
        char *string = active_db_print_all(service->name);
    ...
    {
        tmp_service = initng_active_db_find_by_name(string);
...
static int check_deps(const char **dep_list, const char *dep, int level)
{
    ...
    const char *string = NULL;
    int i, j;
    active_db_h *service = initng_active_db_find_by_name(dep);
