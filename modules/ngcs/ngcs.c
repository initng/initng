...
void docmd(char *cmd, char *arg)
{
	...
	if (strcmp(cmd, "--watch") == 0) {
		...
		res->d.svc_watch.got_svcs = 0;
		res->d.svc_watch.mode = WATCH_START;
		if (ngcs_start_stop(cconn, "start", arg, svc_watch_cb, res) ==
		    NULL) {
			grab_out(NULL);
			printf(C_ERROR "Couldn't send command %s %s\n" C_OFF,
			       cmd, (arg == NULL ? "" : arg));
		...
		res->d.svc_watch.got_svcs = 0;
		res->d.svc_watch.mode = WATCH_STOP;
		if (ngcs_start_stop(cconn, "stop", arg, svc_watch_cb, res) ==
		    NULL) {
			grab_out(NULL);
			printf(C_ERROR "Couldn't send command %s %s\n" C_OFF,
			       cmd, (arg == NULL ? "" : arg));
...
int main(int argc, char *argv[])
{
	...
	if (debug == FALSE && getuid() != 0) {
		printf(C_ERROR "You need root access to communicate with "
		       "initng." C_OFF "\n");
		exit(2);
	}
